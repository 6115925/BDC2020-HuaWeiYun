...
test.to_csv('new/B_testY444.csv',index=False)
...
def build_model(train, test, pred, label, seed=1080, is_shuffle=True):
    ...
    for n_fold, (train_idx, valid_idx) in enumerate(kf_way, start=1):
        ...
        clf.save_model('model' + str(n_fold) + '.txt')
...
result.to_csv('data/B_resultbase8.help.csv', index=False)
result = result.drop_duplicates(subset=['loadingOrder'],keep='first')
result.to_csv('data/B_base8.help.csv', index=False)